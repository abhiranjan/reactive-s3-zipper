...
final class ZipToStreamFlow(bufferSize: Int)
    extends GraphStage[FlowShape[ZipToStreamFlow.ZipSource, ByteString]] {
  ...
  override def createLogic(inheritedAttributes: Attributes): GraphStageLogic =
    new GraphStageLogic(shape) with StageLogging {
      ...
      private def fileChunks(stream: InputStream,
                             buffer: ZipBuffer): Iterator[ByteString] = {
...
object ZipToStreamFlow {
  final case class ZipSource(filePath: String,
                             streamGenerator: () => InputStream)
